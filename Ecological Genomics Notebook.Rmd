...
- Mathematical Theory:
-- A1A2
-- p + q = 1 (allele frequencies adding to 1)
-- HWE: 1 = p^2 + 2pq + q^2
-- Whright-Fisher population: infinitely large, cloased and unfragmented, random mating, no natural selection
...
- dN/dS = (Nmutations/Nsites)/(Synonimous mutations/S sites) = w
if: w >> 1 w>2 there is positive selection
    w = 1 neutral
    w << 1 negative or purifying selections
...
Firt info-update class

To have in mind!

1) Your Questions!

2) Study organisms (Is there a reference genome? How big? Diploid, Hapliod, Polyploids?)

3) Type of Sample (DNA, RNA, combination?)

4) Library: biological library that has not been prepared.
- Ex: Whole genome sequences, RNA Seq, Exom catcher, GBS, Amplicon Sequencing, Associated methods for epigenomics.

5) Sequencing platform: Ilumina for short reads sequencing, Pac Bio and Nano probe for long reads.
...
First tutorial: https://pespenilab.github.io/Ecological-Genomics/Tutorial/2020-01-22_Command_Line_Unix.html

Major calls: 

pwd : to know where do you live on the server

~/ : same thing as typing /users/j/r/jruizaro

cd : change directory

mkdir : make directory

ll : to look what is inside directory

head : to look first rows of a document
...
Second Tutorial: Working with batch scripts

Red spruce: tree species that has undergoes through latitudinal and elevational migration. Now there are unconnected and fragmentated populations in the top of the appalachians going from New York to South Carolina. 
...
4. Map or align to the references genome
